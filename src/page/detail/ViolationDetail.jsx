import React, { PureComponent } from "react";
import { Button, Col, Row, Tag, Image, Input, Collapse, Modal } from "antd";
import { Link } from "react-router-dom";
import TableList from "../../components/tableList/tableList";
import Footer from "../../components/footer/Footer";
import { EditOutlined } from "@ant-design/icons";
import {
  getViolateDetailApi,
  getViolatehistoryListApi,
  getViolateUpdateLicensePlate,
} from "../../api/api";
import "./ViolationDetail.css";

const { Panel } = Collapse;
const imgsbase64 = [
  "data:image/png;base64,",
  "data:image/png;base64,",
];
class ViolationDetail extends PureComponent {
  state = {
    violatedetail: [],
    dataSource: [],
    btn_show: true,
    isModalOpen: false,
    columns: [
      {
        title: "序号",
        render: (_, e, index) => index + 1,
        align: "center",
        className: "columns",
      },
      {
        title: "车牌号",
        dataIndex: "licensePlate",
        key: "licensePlate",

        align: "center",
        className: "columns",
      },
      {
        title: "监测点",
        dataIndex: "monitorName",
        key: "monitorName",

        align: "center",
        className: "columns",
      },
      {
        title: "车辆总重(吨)",
        dataIndex: "showLoad",
        key: "showLoad",
        render: (showLoad) => showLoad / 1000,
        align: "center",
        className: "columns",
      },

      {
        title: "监测时间",
        dataIndex: "violateTime",
        key: "violateTime",

        align: "center",
        className: "columns",
      },
      {
        title: "状态",
        align: "center",
        className: "columns",
        render: (_, item) => (item.dealState === 1 ? "待审核" : "已审核"),
      },
    ],
    total: 0,
    pageNum: 1,
    pageSize: 10,
  };
  licenseValue = React.createRef(null);
  componentDidMount() {
    this.getviolatedetail();
    document.title = "违章车辆详情";
  }
  getviolatedetail = async () => {
    const { params } = this.props.match;

    const res = await getViolateDetailApi({
      violateId: params.id,
      isViolate: params.monitorId,
    });
    this.setState({ violatedetail: res }, () => {
      this.getViolatehistoryList();
    });
    // if (res.code == 1000) {
    //   this.setState({ violatedetail: res.data }, () => {
    //
    //   });
    // }
  };
  getViolatehistoryList = async () => {
    const { params } = this.props.match;
    console.log(this.state.violatedetail.licensePlate);
    const res = await getViolatehistoryListApi({
      licensePlate: this.state.violatedetail.licensePlate,
      isViolate: params.monitorId,
      pageNum: this.state.pageNum,
      pageSize: this.state.pageSize,
    });
    this.setState({ dataSource: res.list, total: res.total });
  };
  //关闭
  colos = () => {
    window.open("about:blank", "_self").close();
  };

  //分页器回调
  Pagination = (page, pageSize) => {
    this.setState({ pageNum: page, pageSize: pageSize }, () => {
      this.getviolatedetailApi();
    });
  };

  licenseColor = () => {
    console.log(this.state.violatedetail.licenseColor);
    switch (this.state.violatedetail.licenseColor) {
      case "黄":
        return "yellow";
      case "蓝":
        return "blue";
      case "绿":
        return "green";
      case "白":
        return "green";
      default:
        return "red";
    }
  };
  changeLicense = () => {
    const { params } = this.props.match;
    const value = this.licenseValue.current.input.defaultValue;
    if (value === "") {
      return;
    }
    getViolateUpdateLicensePlate({
      violateId: params.id,
      newLicensePlate: value,
    }).then((res) => {
      this.setState({ isModalOpen: false });
      this.getviolatedetail();
    });
  };
  render() {
    const { violatedetail, columns, dataSource, total } = this.state;

    var path = {
      pathname: "/print",
      state: violatedetail,
    };
    return (
      <div
        style={{ height: "100%", width: "100%" }}
        className="violationdetail"
      >
        <div className="btn">
          <Row>
            <Col span={2}>
              <Tag className="col-tag">详情页 {">>"}</Tag>
            </Col>
            <Col
              span={22}
              style={{ display: "flex", justifyContent: "flex-end" }}
            >
              <Button type="primary" style={{ marginRight: "8px" }}>
                <Link to={path}>打印预览</Link>
              </Button>
              <Button type="primary" danger onClick={this.colos}>
                关闭
              </Button>
            </Col>
          </Row>
        </div>
        <div className="box">
          <div className="news">
            <div className="one">
              <div className="car_information">
                <div style={{ display: "flex", alignItems: "center" }}>
                  <span
                    className={`car_num ${
                      // vehicledetail.licenseColor === "蓝" ? "blue" : "yellow"
                      this.licenseColor()
                    }
                    `}
                  >
                    {violatedetail.licensePlate}
                  </span>
                  <Button
                    type="link"
                    onClick={() => this.setState({ isModalOpen: true })}
                  >
                    车牌号纠正
                  </Button>
                </div>
                <div className="monitor_news">
                  <div>
                    违章地点:&nbsp;&nbsp;
                    <span>
                      <label
                        style={{ color: "rgb(97, 210, 245)", fontSize: "20px" }}
                      >
                        {violatedetail.monitorAddr}
                      </label>
                    </span>
                  </div>
                  <div>
                    详细地址:&nbsp;&nbsp;
                    <span> {violatedetail.monitorName}</span>
                  </div>
                  <div>
                    监测时间:&nbsp;&nbsp;
                    <span>{violatedetail.violateTime}</span>
                  </div>
                  <div>
                    行驶车道:&nbsp;&nbsp;
                    <span>{violatedetail.devChnnum}车道</span>
                  </div>
                  <div>
                    行驶方向:&nbsp;&nbsp;
                    <span>
                      {violatedetail.driveDirection === null
                        ? "逆行"
                        : violatedetail.driveDirection}
                    </span>
                  </div>
                </div>
              </div>
              <div className="monitor_information">
                <div className="axle_load">
                  <div>
                    限定总重: &nbsp;&nbsp;
                    {this.state.btn_show ? (
                      <>
                        <span>{violatedetail.approvedLoad / 1000}吨</span>
                        <Button
                          type="text"
                          icon={<EditOutlined />}
                          style={{ color: "#06c" }}
                          onClick={() => this.setState({ btn_show: false })}
                        >
                          编辑
                        </Button>
                      </>
                    ) : (
                      <>
                        <Input
                          defaultValue={violatedetail.approvedLoad}
                          style={{ width: "130px" }}
                        ></Input>
                        <Button
                          type="text"
                          style={{ color: "#06c" }}
                          onClick={() => this.setState({ btn_show: true })}
                        >
                          确定
                        </Button>
                        <Button
                          type="text"
                          style={{ color: "#06c" }}
                          onClick={() => this.setState({ btn_show: true })}
                        >
                          取消
                        </Button>
                      </>
                    )}
                  </div>
                  <div>
                    测量重量: &nbsp;&nbsp;
                    <span>{violatedetail.showLoad / 1000}吨</span>
                  </div>
                  <div>
                    超限重量: &nbsp;&nbsp;
                    <span>
                      {(violatedetail.showLoad - violatedetail.approvedLoad) /
                        1000}
                      吨
                    </span>
                  </div>

                  <div>
                    超载率: &nbsp;&nbsp;
                    <span>{violatedetail.overRate}%</span>
                  </div>
                  <div>
                    平均速度: &nbsp;&nbsp;
                    <span>{violatedetail.velocity}km/h</span>
                  </div>
                </div>
                <div className="axle_weight">
                  <Collapse ghost expandIconPosition={"end"}>
                    <Panel header="各轴称重信息">
                      {violatedetail.alexs?.map((item) => {
                        return (
                          <p key={item.num}>
                            {item.weight}轴重{item.wheelbase}千克
                          </p>
                        );
                      })}
                    </Panel>
                  </Collapse>
                </div>
              </div>
            </div>
            <div className="two">
              <Image.PreviewGroup>
                <div className="car_img">
                  <Image
                    style={{ height: "100%", width: "100%" }}
                    src={
                      violatedetail.frontPhotoUrl
                        ? violatedetail.frontPhotoUrl
                        : imgsbase64[0]
                    }
                  />
                </div>
                <div className="car_img">
                  <Image
                    style={{ height: "100%", width: "100%" }}
                    src={
                      violatedetail.fullviewPhotoUrl
                        ? violatedetail.fullviewPhotoUrl
                        : imgsbase64[0]
                    }
                  />
                </div>
                <div className="car_img">
                  <Image
                    style={{ height: "100%", width: "100%" }}
                    src={
                      violatedetail.rearPhotoUrl
                        ? violatedetail.rearPhotoUrl
                        : imgsbase64[0]
                    }
                  />
                </div>

                <div className="car_img">
                  <Image
                    style={{ height: "100%", width: "100%" }}
                    src={
                      violatedetail.sidePhotoUrl
                        ? violatedetail.sidePhotoUrl
                        : imgsbase64[1]
                    }
                  />
                </div>
              </Image.PreviewGroup>
            </div>
          </div>
          <div className="news">
            <div className="three">
              <div className="threeTitle">
                <span>违章历史记录</span>
                <span>共{total}条</span>
              </div>
              <div className="table">
                <div className="tablelist">
                  <TableList dataSource={dataSource} columns={columns} />
                </div>

                <div className="footer-pos">
                  <Footer total={total} Pagination={this.Pagination} />
                </div>
              </div>
            </div>
            <div className="four">
              <div className="video">
                <video
                  src={violatedetail.videoFtpPath}
                  muted="muted"
                  controls="controls"
                  className="video"
                ></video>
              </div>
            </div>
          </div>
        </div>
        <Modal
          title="纠正车牌号"
          open={this.state.isModalOpen}
          destroyOnClose
          onOk={this.changeLicense}
          onCancel={() => this.setState({ isModalOpen: false })}
        >
          <Input ref={this.licenseValue} placeholder="请输入正确的车牌号" />
        </Modal>
      </div>
    );
  }
}

export default ViolationDetail;
