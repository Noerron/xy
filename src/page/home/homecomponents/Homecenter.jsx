...
import img1 from "../../../asstes/imgs/下载 (3).png";
import img2 from "../../../asstes/imgs/ping.png";
import Pie from "../echarscomponents/Pie";
...
class Homecenter extends Component {
  render() {
    return (
      <div className="home_center column">
        <div className="baidu_map column_item">
          <BaiduMap />
        </div>
        <div className="newImgAndPieChart">
          <div className="new_img column_item">
            <div className="title">
              <img src={img1} alt="" /> 最新超重违章车辆
            </div>
            {/* 轮播图 */}
            <Carousel autoplay>
              {this.props?.dataSource?.map((item) => {
                return (
                  <div key={item.id} className="carousel_box">
                    <Link
                      target="_blank"
                      to={`violationdetail/${item.id}/${item.monitorId}`}
                      rel="opener"
                    >
                      <img
                        src={
                          item.frontPhotoUrl !== "/yjnull"
                            ? item.frontPhotoUrl
                            : imgsbase64[0]
                        }
                        alt=""
          ...
          <div className="new_img column_item">
            <div className="title">
              <img src={img2} alt="" /> 今日违章车型比例
            </div>
            {/* 饼图 */}
            <Pie pie_data={this.props.pie_data} />
          </div>
        </div>
      </div>
    );
